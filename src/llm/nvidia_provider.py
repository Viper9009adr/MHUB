"""Nvidia NIM LLM provider implementation via httpx."""
...
class NvidiaProvider:
    """Nvidia NIM LLM provider using httpx.

    Compatible with Nvidia's hosted NIM API and self-hosted NIM
    instances.
    """
    ...
    async def complete(
        ...
        async with httpx.AsyncClient(timeout=self._timeout) as client:
            try:
                ...
                raise LLMTimeoutError(f"Nvidia NIM request timed out: {exc}") from exc
            ...
            try:
                async with httpx.AsyncClient(timeout=self._timeout) as client:
                    try:
                        ...
                        raise LLMTimeoutError(f"Nvidia NIM stream timed out: {exc}") from exc
                    except httpx.HTTPStatusError as exc:
                        self._handle_http_error(exc)
                    except httpx.RequestError as exc:
                        raise LLMProviderError(f"Nvidia NIM stream request failed: {exc}") from exc
    ...
    def _handle_response(self, resp: httpx.Response) -> LLMResponse:
        ...
        if resp.status_code == 401:
            raise LLMAuthenticationError("Nvidia NIM authentication failed")
        if resp.status_code == 429:
            raise LLMRateLimitError("Nvidia NIM rate limit exceeded")
        if resp.status_code >= 400:
            raise LLMProviderError(
                f"Nvidia NIM error {resp.status_code}: {resp.text}"
            )
    ...
    def _handle_http_error(exc: httpx.HTTPStatusError) -> None:
        ...
        raise LLMProviderError(
            f"Nvidia NIM HTTP {exc.response.status_code}: {exc.response.text}"
        ) from exc
